...
<div class="p-4 space-y-6 pb-24">
    ...
    <div class="u-card rounded-2xl p-5 shadow-sm space-y-5">
        <div class="text-center">
            ...
            <p class="text-xs u-muted"><?= lang('profile_cpw_hint') ?></p>
        </div>

        <?= form_open('profile/change-password', ['class' => 'space-y-4']) ?>

            <div>
                <label for="current_password" class="block text-xs font-bold u-text-2 mb-1.5 uppercase tracking-wide"><?= lang('profile_current_password_label') ?></label>
                <input type="password" id="current_password" name="current_password" required
                       autocomplete="current-password" placeholder="<?= lang('profile_current_password_placeholder') ?>"
            ...
            <div>
                <label for="new_password" class="block text-xs font-bold u-text-2 mb-1.5 uppercase tracking-wide"><?= lang('profile_new_password_label') ?></label>
                <input type="password" id="new_password" name="new_password" required minlength="8"
                       autocomplete="new-password" placeholder="<?= lang('common_min8_placeholder') ?>"
                       class="u-input w-full rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                <?= form_error('new_password', '<p class="mt-1.5 text-xs text-rose-500 font-semibold">', '</p>') ?>
            </div>

            <div>
                <label for="confirm_password" class="block text-xs font-bold u-text-2 mb-1.5 uppercase tracking-wide"><?= lang('profile_confirm_password_label') ?></label>
                <input type="password" id="confirm_password" name="confirm_password" required
                       autocomplete="new-password" placeholder="<?= lang('auth_confirm_placeholder') ?>"
                       class="u-input w-full rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
            ...
            <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold py-3.5 rounded-xl transition flex items-center justify-center gap-2">
                <i class="fas fa-check"></i> <?= lang('profile_cpw_btn') ?>
            </button>
