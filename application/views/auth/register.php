...
<head>
    ...
    <title><?= lang('auth_register') ?> · Synapse</title>
...
<div class="w-full max-w-[480px] min-h-screen mx-auto relative overflow-hidden flex flex-col">
    ...
    <section class="h-[34vh] min-h-[260px] w-full relative flex flex-col items-center justify-center shrink-0">
        ...
        <p class="u-auth-tagline relative z-10 mt-3 text-sm font-medium text-center px-6 leading-relaxed"><?= lang('auth_tagline_register') ?></p>
    </section>
    ...
    <div class="auth-card flex-1 rounded-t-[2.5rem] w-full relative z-20 px-6 py-8 flex flex-col -mt-4">
        ...
        <!-- Register Form -->
        <h2 class="text-xl font-bold u-text mb-1"><?= lang('auth_register_title') ?></h2>
        <p class="text-sm u-text-2 mb-6"><?= lang('auth_register_subtitle') ?></p>
        ...
        <?= form_open('register', ['class' => 'space-y-4']) ?>
            ...
            <div>
                ...
                <p class="text-xs u-muted mt-1.5"><?= lang('auth_invite_hint') ?></p>
            </div>

            <div>
                <label for="phone" class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5 block"><?= lang('auth_phone_label') ?> <span class="text-rose-500">*</span></label>
            ...
            <div>
                <label for="password" class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5 block"><?= lang('auth_password_label') ?> <span class="text-rose-500">*</span></label>
                <input type="password" id="password" name="password"
                       class="u-input h-14 px-5 rounded-2xl text-sm focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all w-full"
                       placeholder="<?= lang('auth_min8_placeholder') ?>">
                <?= form_error('password', '<p class="text-xs text-rose-500 mt-1.5">', '</p>') ?>
            </div>

            <!-- Native SVG CAPTCHA (plan/72): inline SVG, single-use, TTL 3 menit -->
            <div>
                <label for="captcha" class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5 block"><?= lang('auth_captcha_label') ?> <span class="text-rose-500">*</span></label>
                <div class="flex items-center gap-3">
                    <input type="text" id="captcha" name="captcha" maxlength="5" autocomplete="off"
                           ...
                           placeholder="<?= lang('auth_captcha_placeholder') ?>" aria-label="<?= lang('auth_captcha_label') ?>"
                    ...
                    <button type="button" id="captcha-refresh" title="<?= lang('auth_captcha_refresh') ?>"
                            aria-label="<?= lang('auth_captcha_refresh') ?>"
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1.5"><?= lang('auth_captcha_hint') ?></p>
            </div>

            <button type="submit" class="u-btn-dark h-14 w-full rounded-2xl shadow-lg flex items-center justify-center mt-2">
                <span><?= lang('auth_register_btn') ?></span>
            </button>

        <?= form_close() ?>

        <p class="u-text-2 text-sm text-center mt-6">
            <?= lang('auth_have_account') ?>
            <a href="<?= site_url('login') ?>" class="text-blue-600 dark:text-blue-400 font-semibold hover:underline"><?= lang('auth_sign_in_link') ?></a>
        </p>
    </div>
