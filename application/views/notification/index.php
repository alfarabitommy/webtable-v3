...
<div class="px-4 pt-4 pb-6">
    ...
    <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-2">
            ...
            <h2 class="text-base font-bold text-slate-900">Riwayat Notifikasi</h2>
        </div>
        <?php if (!empty($notifications)): ?>
            <button onclick="markAllRead()" id="mark-all-btn"
                    class="text-[11px] font-medium px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors active:scale-95 border border-slate-200">
    ...
    <div id="notif-container" class="space-y-2">
        <?php if (empty($notifications)): ?>
            ...
            <div class="flex flex-col items-center justify-center py-16">
                ...
                <p class="text-sm font-medium text-slate-400">Belum ada notifikasi</p>
                <p class="text-xs text-slate-300 mt-1">Notifikasi akan muncul di sini</p>
            </div>
        <?php else: ?>
            <?php
            $type_styles = [
                'info'        => ['border' => 'border-l-blue-500',    'dot' => 'bg-blue-500',    'icon' => 'fa-info-circle',   'icon_color' => 'text-blue-500',    'bg' => 'bg-blue-50',    'pill' => 'bg-blue-100 text-blue-600'],
                'success'     => ['border' => 'border-l-emerald-500', 'dot' => 'bg-emerald-500', 'icon' => 'fa-check-circle',  'icon_color' => 'text-emerald-500', 'bg' => 'bg-emerald-50', 'pill' => 'bg-emerald-100 text-emerald-600'],
                'warning'     => ['border' => 'border-l-amber-500',   'dot' => 'bg-amber-500',   'icon' => 'fa-exclamation-triangle', 'icon_color' => 'text-amber-500', 'bg' => 'bg-amber-50', 'pill' => 'bg-amber-100 text-amber-600'],
                'error'       => ['border' => 'border-l-rose-500',    'dot' => 'bg-rose-500',    'icon' => 'fa-times-circle',  'icon_color' => 'text-rose-500',    'bg' => 'bg-rose-50',    'pill' => 'bg-rose-100 text-rose-600'],
                'commission'  => ['border' => 'border-l-indigo-500',  'dot' => 'bg-indigo-500',  'icon' => 'fa-coins',         'icon_color' => 'text-indigo-500',  'bg' => 'bg-indigo-50',  'pill' => 'bg-indigo-100 text-indigo-600'],
            ];
            $type_labels = [
                'info'       => 'Info',
                'success'    => 'Berhasil',
            ...
            foreach ($notifications as $n):
                ...
                ?>
                    ...
                    <div class="flex items-center gap-3 py-2 mt-2 first:mt-0">
                        ...
                        <span class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider"><?= $date_label ?></span>
                        <div class="flex-1 h-px bg-slate-200"></div>
                    </div>
                <?php endif; ?>
                ...
                <div class="notif-card <?= $style['border'] ?> border-l-[3px] rounded-lg transition-all duration-200
                    <?= $is_unread ? 'bg-slate-50 border border-slate-100 shadow-sm' : 'bg-white border border-slate-100' ?>"
                    ...
                    <div class="flex items-start gap-3 p-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-0.5">
                                ...
                                <?php endif; ?>
                                <h4 class="text-sm font-semibold <?= $is_unread ? 'text-slate-900' : 'text-slate-600' ?> leading-tight truncate">
                            ...
                            <p class="text-xs <?= $is_unread ? 'text-slate-600' : 'text-slate-400' ?> mt-0.5 leading-relaxed">
    ...
    .then(data => {
        if (data.success) {
            ...
            document.querySelectorAll('.notif-card[data-unread="1"]').forEach(card => {
                card.classList.remove('bg-slate-50', 'shadow-sm', 'border-slate-100');
            ...
            if (btn) {
                ...
                btn.classList.remove('bg-slate-100', 'hover:bg-slate-200', 'text-slate-600');
