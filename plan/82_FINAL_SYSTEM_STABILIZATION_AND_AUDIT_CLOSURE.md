...
4. **Production DB verification.** Run `database.sql` diff against the live DB (canonical 12-table inventory + `rentals`/`otp_logs` deprecated retention tables; `user_rentals` expiry indexes present). Confirm `rate_limits`, `system_settings` financial keys, and the canonical 8-package `gpu_products` seed (id 1–8, plan/82 lineup) exist.
